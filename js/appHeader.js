...
export const createHeader = (city) => {
    ...
    cityLocation.textContent='Сменить местоположение';
...
//проверка, заполнено ли поле "сменить город"
    searchBtn.addEventListener('click', async() => {
        ...
        try {
            ...
            //но если приходит ошибка из openweather,вызываем showError

            if (weather.message) {
                showError(weather.message);
                return; //что бы код перестал выполняться
            }
    ...
    //кнопки переключения с C на F и наоборот.
    ...
    //кликая по любому месту инпут пропадает
    ...
    return header; //функция возвращает готовый header
}